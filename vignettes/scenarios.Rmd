...
# setting some identical matching to FALSE can help compare scenarios where
# key parameters are *expected* to be different, but where some parameters
# must be *identical*
sce_are_comparable(
  baseline = low_r0_data,
  compare = high_r0_data,
  match_variables = c("age_groups", "r0"),
  comparison_variables = "p_infected",
  expect_identical_match = c(
    r0 = FALSE,
    age_groups = TRUE # age group limits must be identical for valid comparison
  )
)
```
